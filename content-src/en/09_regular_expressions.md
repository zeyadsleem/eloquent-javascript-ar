...
![Illustration of a railroad system representing the syntactic structure of regular expressions](/images/book/chapter_picture_9.jpg)
...
That regular expression looks completely awful, doesn’t it? Half of it is backslashes, producing a background noise that makes it hard to spot the actual pattern expressed. We’ll see a slightly improved version of this expression [later](/chapter/regular_expressions#date_regexp_counted).
...
| `\p{Script=Hangul}` | Any character from the given script (see [Chapter 5](/chapter/higher_order_functions#scripts)) |
...
![Railroad diagram for the regular expression '^([01]+b|\d+|[\da-f]+h)$'](/images/book/re_number.svg)
...
![Railroad diagram for the regular expression '([01]+)+b'](/images/book/re_slow.svg)
...
Refer to the table in the [chapter summary](/chapter/regular_expressions#summary_regexp) for help. Test each solution with a few test strings.
