...
We saw the word *map* used in the [previous chapter](/chapter/higher_order_functions#map) for an operation that transforms a data structure by applying a function to its elements. Confusing as it is, in programming the same word is used for a related but rather different thing.
...
Let’s implement an iterable data structure similar to the linked list from the exercise in [Chapter 4](/chapter/data_structures_objects_and_arrays). We’ll write the list as a class this time.
