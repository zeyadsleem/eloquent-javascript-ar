...
![Illustration showing two crows on a tree branch](/images/book/chapter_picture_11.jpg)
...
When you define a function with `function*` (placing an asterisk after the word `function`), it becomes a generator. When you call a generator, it returns an iterator, which we already saw in [Chapter 6](/chapter/the_secret_life_of_objects).
...
Writing iterators is often much easier when you use generator functions. The iterator for the `Group` class (from the exercise in [Chapter 6](/chapter/the_secret_life_of_objects#group_iterator)) can be written with this generator:
