...
![Illustration showing a telephone pole with a tangle of wires going in all directions](/images/book/chapter_picture_20.jpg)

So far, we’ve used the JavaScript language in a single environment: the browser. This chapter and the [next one](/chapter/project_skill_sharing_website) will briefly introduce Node.js, a program that allows you to apply your JavaScript skills outside of the browser. With it, you can build anything from small command line tools to HTTP servers that power dynamic websites.
...
If you want to follow along and run the code in this chapter, you’ll need to install Node.js version 18 or higher. To do so, go to [*https://nodejs.org*](https://nodejs.org) and follow the installation instructions for your operating system. You can also find further documentation for Node.js there.
...
Node started out using the CommonJS module system, based on the `require` function, which we saw in [Chapter 10](/chapter/modules#commonjs). It will still use this system by default when you load a `.js` file.
...
NPM, introduced in [Chapter 10](/chapter/modules#modules_npm), is an online repository of JavaScript modules, many of which are specifically written for Node. When you install Node on your computer, you also get the `npm` command, which you can use to interact with this repository.

NPM’s main use is downloading packages. We saw the `ini` package in [Chapter 10](/chapter/modules#modules_ini). We can use NPM to fetch and install that package on our computer.
...
The robot simulation from [Chapter 7](/chapter/project_a_robot), as modularized in the exercise in [Chapter 10](/chapter/modules#modular_robot), might have a `package.json` file like this:
...
This book won’t delve further into the details of NPM usage. Refer to [*https://npmjs.com*](https://npmjs.com) for further documentation and a way to search for packages.
...
The `node:fs` module contains many other useful functions: `readdir` will give you the files in a directory as an array of strings, `stat` will retrieve information about a file, `rename` will rename a file, `unlink` will remove one, and so on. See the documentation at [*https://nodejs.org*](https://nodejs.org) for specifics.

Most of these take a callback function as the last parameter, which they call either with an error (the first argument) or with a successful result (the second). As we saw in [Chapter 11](/chapter/asynchronous_programming), there are downsides to this style of programming—the biggest one being that error handling becomes verbose and error prone.
...
If you run this script on your own machine, you can point your web browser at [*http://localhost:8000/hello*](http://localhost:8000/hello) to make a request to your server. It will respond with a small HTML page.
...
To send something to the client, you call methods on the `response` object. The first, `writeHead`, will write out the response headers (see [Chapter 18](/chapter/http_and_forms#headers)). You give it the status code (200 for “OK” in this case) and an object that contains header values. The example sets the `Content-Type` header to inform the client that we’ll be sending back an HTML document.
...
Use an HTML form to edit the content of the files that make up the website, allowing the user to update them on the server by using HTTP requests, as described in [Chapter 18](/chapter/http_and_forms).
...
You can create a `<textarea>` element to hold the content of the file that is being edited. A `GET` request, using `fetch`, can retrieve the current content of the file. You can use relative URLs like *index.html*, instead of [*http://localhost:8000/index.html*](http://localhost:8000/index.html), to refer to files on the same server as the running script.
