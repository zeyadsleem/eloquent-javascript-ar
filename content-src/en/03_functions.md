...
![Illustration of fern leaves with a fractal shape, bees in the background](/images/book/chapter_picture_3.jpg)
...
Each binding has a *scope*, which is the part of the program in which the binding is visible. For bindings defined outside of any function, block, or module (see [Chapter 10](/chapter/modules)), the scope is the whole program—you can refer to such bindings wherever you want. These are called *global*.
...
In [Chapter 5](/chapter/higher_order_functions), we’ll discuss the interesting things that we can do by passing function values to other functions.
...
There’s no deep reason to have both arrow functions and `function` expressions in the language. Apart from a minor detail, which we’ll discuss in [Chapter 6](/chapter/the_secret_life_of_objects), they do the same thing. Arrow functions were added in 2015, mostly to make it possible to write small function expressions in a less verbose way. We’ll use them often in [Chapter 5](/chapter/higher_order_functions).
...
The [next chapter](/chapter/data_structures_objects_and_arrays#rest_parameters) will introduce a way in which a function body can get at the whole list of arguments it was passed. This is helpful because it allows a function to accept any number of arguments. For example, `console.log` does this, outputting all the values it is given:
...
This is rather close to the way mathematicians define exponentiation and arguably describes the concept more clearly than the loop we used in [Chapter 2](/chapter/program_structure). The function calls itself multiple times with ever smaller exponents to achieve the repeated multiplication.
...
The [previous chapter](/chapter/program_structure#return_values) introduced the standard function `Math.min` that returns its smallest argument. We can write a function like that ourselves now. Define the function `min` that takes two arguments and returns their minimum.
