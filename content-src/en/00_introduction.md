...
![Illustration of a screwdriver next to a circuit board of about the same size](/images/book/chapter_picture_00.jpg)
...
The moral of this story is that the same program can be expressed in both long and short, unreadable and readable ways. The first version of the program was extremely obscure, whereas this last one is almost English: `log` the `sum` of the `range` of numbers from 1 to 10. (We will see in [later chapters](/chapter/data_structures_objects_and_arrays) how to define operations like `sum` and `range`.)
...
Web browsers are not the only platforms on which JavaScript is used. Some databases, such as MongoDB and CouchDB, use JavaScript as their scripting and query language. Several platforms for desktop and server programming, most notably the Node.js project (the subject of [Chapter 20](/chapter/node_js)), provide an environment for programming JavaScript outside of the browser.
...
The language part of the book starts with four chapters that introduce the basic structure of the JavaScript language. They discuss [control structures](/chapter/program_structure) (such as the `while` word you saw in this introduction), [functions](/chapter/functions) (writing your own building blocks), and [data structures](/chapter/data_structures_objects_and_arrays). After these, you will be able to write basic programs. Next, Chapters [5](/chapter/higher_order_functions) and [6](/chapter/the_secret_life_of_objects) introduce techniques to use functions and objects to write more *abstract* code and keep complexity under control.

After a [first project chapter](/chapter/project_a_robot) that builds a crude delivery robot, the language part of the book continues with chapters on [error handling and bug fixing](/chapter/bugs_and_errors), [regular expressions](/chapter/regular_expressions) (an important tool for working with text), [modularity](/chapter/modules) (another defense against complexity), and [asynchronous programming](/chapter/asynchronous_programming) (dealing with events that take time). The [second project chapter](/chapter/project_a_programming_language), where we implement a programming language, concludes the first part of the book.

The second part of the book, Chapters [13](/chapter/javascript_and_the_browser) to [19](/chapter/project_a_pixel_art_editor), describes the tools that browser JavaScript has access to. You’ll learn to display things on the screen (Chapters [14](/chapter/the_document_object_model) and [17](/chapter/drawing_on_canvas)), respond to user input ([Chapter 15](/chapter/handling_events)), and communicate over the network ([Chapter 18](/chapter/http_and_forms)). There are again two project chapters in this part: building a [platform game](/chapter/project_a_platform_game) and a [pixel paint program](/chapter/project_a_pixel_art_editor).

[Chapter 20](/chapter/node_js) describes Node.js, and [Chapter 21](/chapter/project_skill_sharing_website) builds a small website using that tool.
