...
![Illustration showing an industrial-looking robot arm drawing a city on a piece of paper](/images/book/chapter_picture_17.jpg)

Browsers give us several ways to display graphics. The simplest way is to use styles to position and color regular DOM elements. This can get us quite far, as the game in the [previous chapter](/chapter/project_a_platform_game) showed. By adding partially transparent background images to the nodes, we can make them look exactly the way we want. It is even possible to rotate or skew nodes with the `transform` style.
...
This book won’t go into SVG in detail, but I’ll briefly explain how it works. At the [end of the chapter](/chapter/drawing_on_canvas#graphics_tradeoffs), I’ll come back to the trade-offs that you must consider when deciding which drawing mechanism is appropriate for a given application.
...
We’ll come back to our pie chart, and the problem of labeling the slices, in the [exercises](/chapter/drawing_on_canvas#exercise_pie_chart) at the end of the chapter.
...
![Pixel art showing a computer game character in 10 different poses. The first 8 form its running animation cycle, the 9th has the character standing still, and the 10th shows him jumping.](/images/book/player_big.png)
...
![Diagram showing the effect of translating and mirroring a triangle](/images/book/mirror.svg)
...
We now know enough about canvas drawing to start working on a canvas-based display system for the game from the [previous chapter](/chapter/project_a_platform_game). The new display will no longer be showing just colored boxes. Instead, we’ll use `drawImage` to draw pictures that represent the game’s elements.

We define another display object type called `CanvasDisplay`, supporting the same interface as `DOMDisplay` from [Chapter 16](/chapter/project_a_platform_game#domdisplay)—namely, the methods `syncState` and `clear`.
...
To draw the background, we run through the tiles that are visible in the current viewport, using the same trick used in the `touches` method from the [previous chapter](/chapter/project_a_platform_game#touches).
...
![Pixel art showing three sprites: a piece of wall, made out of small white stones, a square of orange lava, and a round coin.](/images/book/sprites_big.png)
...
We don’t bother waiting for the sprite image to load. Calling `drawImage` with an image that hasn’t been loaded yet will simply do nothing. Thus, we might fail to draw the game properly for the first few frames while the image is still loading, but that isn’t a serious problem. Since we keep updating the screen, the correct scene will appear as soon as the loading finishes.
...
![Picture showing the shapes you are asked to draw](/images/book/exercise_shapes.png)

When drawing the last two shapes, you may want to refer to the explanation of `Math.cos` and `Math.sin` in [Chapter 14](/chapter/the_document_object_model#sin_cos), which describes how to get coordinates on a circle using these functions.
...
If you are not sure how to find out which side of the circle a given angle is on, look to the explanation of `Math.cos` in [Chapter 14](/chapter/the_document_object_model#sin_cos). The cosine of an angle tells us which x-coordinate it corresponds to, which in turn tells us exactly which side of the circle we are on.

</details>

### A bouncing ball

Use the `requestAnimationFrame` technique that we saw in [Chapter 14](/chapter/the_document_object_model#animationFrame) and [Chapter 16](/chapter/project_a_platform_game#runAnimation) to draw a box with a bouncing ball in it. The ball moves at a constant speed and bounces off the box’s sides when it hits them.
...
To model the ball’s position and speed, you can use the `Vec` class from [Chapter 16](/chapter/project_a_platform_game#vector) (which is available on this page). Give it a starting speed, preferably one that is not purely vertical or horizontal, and for every frame multiply that speed by the amount of time that elapsed. When the ball gets too close to a vertical wall, invert the *x* component in its speed. Likewise, invert the *y* component when it hits a horizontal wall.
