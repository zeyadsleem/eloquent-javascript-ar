...
![Illustration of a complicated building built from modular pieces](/images/book/chapter_picture_10.jpg)
...
Module interfaces have a lot in common with object interfaces, as we saw them in [Chapter 6](/chapter/the_secret_life_of_objects#interface). They make part of the module available to the outside world and keep the rest private.
...
An example of a slightly more complex data structure is the graph from [Chapter 7](/chapter/project_a_robot). There is no single obvious way to represent a graph in JavaScript. In that chapter, we used an object whose properties hold arrays of strings—the other nodes reachable from that node.
...
These are the bindings that the project from [Chapter 7](/chapter/project_a_robot) creates:
...
The `require` function given [earlier in this chapter](/chapter/modules#require) supports this type of dependency cycle. Can you see how it handles cycles?
