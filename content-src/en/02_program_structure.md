...
![Illustration showing a number of tentacles holding chess pieces](/images/book/chapter_picture_2.jpg)
...
In [Chapter 1](/chapter/values_types_and_operators), we made values and applied operators to them to get new values. Creating values like this is the main substance of any JavaScript program. But that substance has to be framed in a larger structure to be useful. That’s what we’ll cover in this chapter.
...
The first of these, `var` (short for “variable”), is the way bindings were declared in pre-2015 JavaScript, when `let` didn’t exist yet. I’ll get back to the precise way it differs from `let` in the [next chapter](/chapter/functions). For now, remember that it mostly does the same thing, but we’ll rarely use it in this book because it behaves oddly in some situations.
...
![A prompt dialog that says 'enter passcode'](/images/book/prompt.png)
...
[Chapter 3](/chapter/functions) will explain how to write your own functions.
...
![Diagram showing a straight arrow](/images/book/controlflow-straight.svg)

## Conditional execution

Not all programs are straight roads. We may, for example, want to create a branching road where the program takes the proper branch based on the situation at hand. This is called *conditional execution*.

![Diagram of an arrow that splits in two, and then rejoins again](/images/book/controlflow-if.svg)
...
![Diagram showing arrow that splits in two, with on the branches splitting again, before all branches rejoin again](/images/book/controlflow-nested-if.svg)
...
![Diagram showing an arrow to a point which has a cyclic arrow going back to itself and another arrow going further](/images/book/controlflow-loop.svg)
...
The counter could also have started at `1` and checked for `<= 10`, but for reasons that will become apparent in [Chapter 4](/chapter/data_structures_objects_and_arrays#array_indexing), it is a good idea to get used to counting from 0.
...
This program is exactly equivalent to the [earlier](/chapter/program_structure#loops) even-number-printing example. The only change is that all the statements that are related to the “state” of the loop are grouped together after `for`.
...
If you are unsure how to test your solutions to the exercises, refer to the [introduction](/chapter/introduction).

Each exercise starts with a problem description. Read this description and try to solve the exercise. If you run into problems, consider reading the hints after the exercise. You can find full solutions to the exercises online at [*https://eloquentjavascript.net/code*](https://eloquentjavascript.net/code#2). If you want to learn something from the exercises, I recommend looking at the solutions only after you’ve solved the exercise, or at least after you’ve attacked it long and hard enough to have a slight headache.
...
You can start with a program that prints out the numbers 1 to 7, which you can derive by making a few modifications to the [even number printing example](/chapter/program_structure#loops) given earlier in the chapter, where the `for` loop was introduced.
