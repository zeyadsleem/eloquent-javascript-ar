...
![Illustration showing a Rube Goldberg machine involving a ball, a see-saw, a pair of scissors, and a hammer, which affect each other in a chain reaction that turns on a lightbulb.](/images/book/chapter_picture_15.jpg)
...
We’ll create a less primitive drawing application in [Chapter 19](/chapter/project_a_pixel_art_editor).
...
In the context of the event loop, as discussed in [Chapter 11](/chapter/asynchronous_programming), browser event handlers behave like other asynchronous notifications. They are scheduled when the event occurs but must wait for other scripts that are running to finish before they get a chance to run.
...
In this exercise, I want you to implement a mouse trail. Use absolutely positioned `<div>` elements with a fixed size and background color (refer to the [code](/chapter/handling_events#mouse_drawing) in the “Mouse Clicks” section for an example). Create a bunch of these elements and, when the mouse moves, display them in the wake of the mouse pointer.
