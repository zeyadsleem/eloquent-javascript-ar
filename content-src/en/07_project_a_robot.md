...
![Illustration of a robot holding a stack of packages](/images/book/chapter_picture_7.jpg)
...
![Pixel art illustration of a small village with 11 locations, labeled with letters, and roads going being them](/images/book/village2x.png)
...
The way `runRobotAnimation` is implemented will remain a mystery for now, but after you’ve read the [later chapters](/chapter/the_document_object_model) of this book, which discuss JavaScript integration in web browsers, you’ll be able to guess how it works.
...
The problem of finding a route through a graph is a typical *search problem*. We can tell whether a given solution (a route) is valid, but we can’t directly compute the solution the way we could for 2 + 2. Instead, we have to keep creating potential solutions until we find one that works.
...
Write a new class `PGroup`, similar to the `Group` class from [Chapter 6](/chapter/the_secret_life_of_objects#groups), which stores a set of values. Like `Group`, it has `add`, `delete`, and `has` methods. Its `add` method, however, should return a *new* `PGroup` instance with the given member added and leave the old one unchanged. Similarly, `delete` should create a new instance without a given member.
