...
![Illustration of a sea of dark and bright dots (bits) with islands in it](/images/book/chapter_picture_1.jpg)
...
There’s a complication though: JavaScript’s representation uses 16 bits per string element, which can describe up to 216 different characters. However, Unicode defines more characters than that—about twice as many, at this point. So some characters, such as many emoji, take up two “character positions” in JavaScript strings. We’ll come back to this in [Chapter 5](/chapter/higher_order_functions#code_units).
...
String values have a number of associated functions (*methods*) that can be used to perform other operations on them. I’ll say more about these in [Chapter 4](/chapter/data_structures_objects_and_arrays#methods).
...
We will use `console.log` in example code to indicate that we want to see the result of evaluating something. (More about that in the [next chapter](/chapter/program_structure).)
...
In the [introduction](/chapter/introduction), I mentioned that JavaScript goes out of its way to accept almost any program you give it, even programs that do odd things. This is nicely demonstrated by the following expressions:
...
This gives you enough information to use JavaScript as a pocket calculator but not much more. The [next chapter](/chapter/program_structure) will start tying these expressions together into basic programs.
