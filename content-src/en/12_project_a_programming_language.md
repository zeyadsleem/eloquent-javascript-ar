...
![Illustration showing an egg with holes in it, showing smaller eggs inside, which in turn have even smaller eggs in them, and so on](/images/book/chapter_picture_12.jpg)
...
![A diagram showing the structure of the syntax tree for the example program. The root is labeled 'do' and has two children, one labeled 'define' and one labeled 'if'. Those in turn have more children, describing their content.](/images/book/syntax_tree.svg)

Contrast this to the parser we wrote for the configuration file format in [Chapter 9](/chapter/regular_expressions#ini), which had a simple structure: it split the input into lines and handled those lines one at a time. There were only a few simple forms that a line was allowed to have.
...
After skipping any leading space, `parseExpression` uses three regular expressions to spot the three atomic elements that Egg supports: strings, numbers, and words. The parser constructs a different kind of data structure depending on which expression matches. If the input does not match one of these three forms, it is not a valid expression, and the parser throws an error. We use the `SyntaxError` constructor here. This is an exception class defined by the standard, like `Error`, but more specific.
...
We use plain JavaScript function values to represent Egg’s function values. We will come back to this [later](/chapter/project_a_programming_language#egg_fun), when the special form `fun` is defined.
