...
<svelte:head>
  <title>البحث في الكتاب | Eloquent JavaScript</title>
</svelte:head>

<div class="container page">
  <h1>البحث في الكتاب</h1>
  ...
  {#if query.length >= 2}
    ...
    <div class="search-results">
      {#each results as result}
        <a class="search-result" href={chapterPath(result.chapter.slug)}>
          <strong
            >الفصل {result.chapter.number}: {result.chapter.title}</strong
          >
